...
def DrawMatrix_111(x, y, matL, matCb, matCr):
    """
...
def DrawMatrix_411(x, y, matL, matCb, matCr):
    ...
    Cb = np.zeros((4,8,8), dtype=int)
    for i in range(2):
        for j in range(2):
            for yy in range(4):
                for xx in range(4):
                    Cb[2*i+j][2*yy][2*xx] = matCb[4*i+yy][4*j+xx]
                    Cb[2*i+j][2*yy][2*xx+1] = matCb[4*i+yy][4*j+xx]
                    Cb[2*i+j][2*yy+1][2*xx] = matCb[4*i+yy][4*j+xx]
                    Cb[2*i+j][2*yy+1][2*xx+1] = matCb[4*i+yy][4*j+xx]         
              
    Cr = np.zeros((4,8,8), dtype=int)
    for i in range(2):
        for j in range(2):
            for yy in range(4):
                for xx in range(4):
                    Cr[2*i+j][2*yy][2*xx] = matCr[4*i+yy][4*j+xx]
                    Cr[2*i+j][2*yy][2*xx+1] = matCr[4*i+yy][4*j+xx]
                    Cr[2*i+j][2*yy+1][2*xx] = matCr[4*i+yy][4*j+xx]
                    Cr[2*i+j][2*yy+1][2*xx+1] = matCr[4*i+yy][4*j+xx]         
...
class JPEG:
    ...
    def DefineQuantizationTables(self, data, db_cnt):
        if db_cnt == 67:
            loop = 1
        elif db_cnt == 132:
            loop = 2
    ...
    def StartOfScan(self, data, hdrlen):
        ...
        mcu_vCnt = math.ceil(self.height / (8 * Vmax)) # vertical
        
        for y in range(mcu_vCnt):
            for x in range(mcu_hCnt):
                ...
                if flag == 1:
                    ...
                    DrawMatrix_111(x, y, matL.base, matCb.base, matCr.base)
                    
                elif flag == 2:
                    L = np.zeros((4,8,8), dtype=int)
                    matL, oldlumdccoeff = self.BuildMatrix(st, 0, self.quant[self.quantMapping[0]], oldlumdccoeff)
                    L[0] = matL.base
                    matL, oldlumdccoeff = self.BuildMatrix(st, 0, self.quant[self.quantMapping[0]], oldlumdccoeff)
                    L[1] = matL.base
    ...
    def BaselineDCT(self, data):
        ...
        print("size %ix%i" % (self.height,  self.width))
        ...
        while True:
            ...
            if marker == 0xFFD8:
                ...
                if marker == 0xFFC4:
                    ...
                    self.DefineQuantizationTables(chunk, db_cnt)
                elif marker == 0xFFC0:
